...
MinGW_LINK_FLAGS = "-fmerge-all-constants -fexpensive-optimizations -finline-functions -flto -fno-stack-protector -fno-unroll-loops -fno-exceptions -fno-rtti -s -Wpointer-arith -fpermissive -Wl,--gc-sections -Wl,--strip-all -static-libgcc -static-libstdc++ -shared"
...
def link():
    LOG_OUTPUT("Building the DLL")
    link_command = f"{MinGW} {BUILD_DIR}/syscalls.o {BUILD_DIR}/retaddrspoof.o {BUILD_DIR}/Resource.o {BUILD_DIR}/Ghost.o {MinGW_LINK_FLAGS} -o {BUILD_DIR}/Ghost.dll"
    link = subprocess.run(link_command, shell=True, stdout=subprocess.DEVNULL,stderr=subprocess.PIPE, text=True)
    if (link.stderr):
        print("[ERROR] failed to build Ghost.cpp to DLL")
        print(f"[i] command used: {link_command}")
        exit()
    LOG_OUTPUT("DLL -> build/Ghost.dll")
